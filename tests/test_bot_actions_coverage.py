...
async def test_show_main_menu_uses_user_language(monkeypatch):
    bot = AsyncFakeBot()
    monkeypatch.setattr(actions, "get_user_language", to_async(lambda user_id: "uk"))
...
async def test_send_track_card_sends_photo_when_cover_is_available(monkeypatch, sample_track):
    bot = AsyncFakeBot()
    monkeypatch.setattr(actions, "get_user_language", to_async(lambda user_id: "en"))
    monkeypatch.setattr(actions, "enrich_track_with_spotify_link", to_async(lambda track: track))
    monkeypatch.setattr(actions, "format_track_card", lambda track: "formatted")
    monkeypatch.setattr(actions, "save_last_track_id", to_async(lambda telegram_id, deezer_id: None))
    monkeypatch.setattr(actions, "is_track_favorite", to_async(lambda **kwargs: False))
...
async def test_send_track_card_falls_back_to_message_when_photo_fails(monkeypatch, sample_track):
    ...
    monkeypatch.setattr(actions, "format_track_card", lambda track: "formatted")
    monkeypatch.setattr(actions, "save_last_track_id", to_async(lambda telegram_id, deezer_id: None))
    monkeypatch.setattr(actions, "is_track_favorite", to_async(lambda **kwargs: True))
...
async def test_send_track_card_handles_last_track_id_error(monkeypatch, sample_track):
    ...
    monkeypatch.setattr(actions, "log_and_save_error", to_async(lambda *a, **k: calls.append(1)))
